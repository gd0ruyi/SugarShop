...
class Mongo extends Driver {
	 ...
	 * @var string
	 */
	protected $_inc_collection = 'counters';
	...
	public function __construct($config = '') {
		$this->_inc_collection = C ( 'MONGODB_AUTO_INC_COLL' ) ? C ( 'MONGODB_AUTO_INC_COLL' ) : $this->_inc_collection;
	...
	public function insert($data, $options = array(), $replace = false) {		
		...
		if ($this->config ['debug']) {
			...
			$this->queryStr .= $replace ? '.save(' : '.insert(';
			$this->queryStr .= $data ? json_encode ( $data ) : '{}';
			$this->queryStr .= $options ['_options'] ? json_encode ( $options ['_options'] ) : '{}';
			$this->queryStr .= ')';
		}
		try {
			$this->debug ( true );
			// gd0ruyi@163.com:修正mongodb的save方法
			$result = $replace ? $this->_collection->save ( $data, $options ['_options'] ) : $this->_collection->insert ( $data, $options ['_options'] );
			$this->debug ( false );
			if ($result) {
				$_id = $data ['_id'];
				if (is_object ( $_id )) {
	 ...
	 *
	 * @author gd0ruyi@163.com 2016-01-05
	 * @uses 修正原Thinkphp的MongoDB的自增处理，解决并发时，自增ID会出现重复问题。
	 * @access public
	 * @param string $pk
	 *        	主键名
	 * @return integer
	...
	public function getMongoNextId($pk) {
		...
		if (! $result) {
			...
			$this->insert ( $data, $options, false );
		}
		try {
			$this->debug ( true );
			$query = array (
			...
			if ($this->config ['debug']) {
				$this->queryStr = $this->_dbName . '.' . $this->_collectionName . '.findAndModify(' . json_encode ( $query ) . ',' . json_encode ( $update ) . ',' . json_encode ( $fields ) . ',' . json_encode ( $options ) . ')';
			}
			// 获取自增ID信息
			$result = $this->_collection->findAndModify ( $query, $update, $fields, $options );
			$this->debug ( false );
		} catch ( \MongoCursorException $e ) {
			E ( $e->getMessage () );
		}
	...
	public function update($data, $options) {
		...
		try {
			...
			// gd0ruyi@163.com:加入可控制的mongodb选项
			if (isset ( $options ['_options'] )) {
				$multiple = array_merge ( $multiple, $options ['_options'] );
			}
