...
class ShowPageTraceBehavior
{
    ...
    public function run(&$params)
    {
        ...
        // 用ajax也可以使用
        if (!IS_CLI && C('SHOW_PAGE_TRACE') && isDebug()) {
            echo $this->showTrace();
        }
    }
    ...
    private function showTrace()
    {
        ...
        // 调用Trace页面模板
        ob_start();
        // 修改，支持ajax的debug模版输出,用于_printDebug
        // include C('TMPL_TRACE_FILE') ? C('TMPL_TRACE_FILE') : THINK_PATH . 'Tpl/page_trace.tpl';
        include C('TMPL_TRACE_FILE') ? C('TMPL_TRACE_FILE') : THINK_PATH . (IS_AJAX ? 'Tpl/page_trace_ajax.tpl' : 'Tpl/page_trace.tpl');
        return ob_get_clean();
    }
